...
```bash
docker run --rm -v "$(pwd)/../data:/data" ghcr.io/valhalla/valhalla:latest valhalla_build_tiles -c /data/03_interim/valhalla_data/valhalla.json /data/portugal-latest.osm.pbf
```
...
*   **"File Not Found"**: Double-check that your `portugal-latest.osm.pbf` is exactly in the `data/` folder (not `data/03_interim/valhalla_data/`).
